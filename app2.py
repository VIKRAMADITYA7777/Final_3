# -*- coding: utf-8 -*-
"""
Created on Wed Jan 22 22:46:01 2025
...
# Ensure input is valid and all fields are filled
overd = st.text_input("Overs Bowled (up to 1 decimal place)", "20.0")
...
# Convert inputs and ensure they meet the required conditions
try:
    overd = float(overd)
    if overd < 0 or overd > 50:
        st.error("Overs Bowled must be between 0 and 50.")
        raise ValueError("Invalid overs value")
    overd = round(overd, 1)  # Round to 1 decimal place

    runs_till_now = int(runs_till_now)
    if runs_till_now <= 0:
        st.error("Runs Scored Till Now must be greater than 0.")
        raise ValueError("Invalid runs value")
